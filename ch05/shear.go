...
func main() {
	...
	dstP := gocv.NewPoint2fVectorFromPoints([]gocv.Point2f{{0, 0}, {float32(src.Rows()) * 0.5, float32(src.Rows())}, {float32(src.Cols()), 0}})
	...
	w := src.Cols()
	h := src.Rows()
	gocv.WarpAffine(src, &dst, m, image.Point{X: w + int(float32(h)*0.5), Y: h})
